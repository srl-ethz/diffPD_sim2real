    ...
    def compensate_damping(self, q=None, v=None):
        ...
        for idx in range(0, self.vert_num):
            if idx%2==0:  # for DoFs= 4608
                #f_ext[int(idx),2]=-0.004*v[int(idx),0] # for dt=0.0025
                #f_ext[int(idx),2]=0.014*v[int(idx),0] # for dt=0.005
                #f_ext[int(idx),2]=0.032*v[int(idx),0] # for dt=0.0075
                f_ext[int(idx),2]=0.048*v[int(idx),0] # for dt=0.01
                #f_ext[int(idx),2]=0.061*v[int(idx),0] # for dt=0.0125
                #f_ext[int(idx),2]=0.074*v[int(idx),0] # for dt=0.015
                #f_ext[int(idx),2]=0.087*v[int(idx),0] # for dt=0.0175
                #f_ext[int(idx),2]=0.100*v[int(idx),0] # for dt=0.02
...
if __name__ == '__main__':
    seed = 42
    np.random.seed(seed)
    folder = Path('Numerical_Damping_Compensation_Gravity_V2')
    ...
    start_frame = 65
    #end_frame = 888 #for dt=0.0025
    #end_frame = 475 #for dt=0.005
    #end_frame = 340 #for dt=0.0075
    end_frame = 270 #for dt=0.01  
    #end_frame=233 #for dt=0.0125
    #end_frame=195 #for dt=0.015
    #end_frame=183 #for dt=0.0175
    #end_frame=168 #for dt=0.02
    ...
    for method, opt in zip(methods[:1], opts[:1]):
        ...
        time_q_real = []
        for i in range(frame_num):
            time_q_real.append([np.array(i*0.01)])
        #print_info(f"time: {time}")
        ...
        ax.plot(time_q_real,z_qs, marker='o', markersize=4, label='Real Data')
        ...
        ax.plot(time,z_hex, marker='o', markersize=4, label='Hex Mesh ({} DoFs)'.format(hex_deformable.dofs()))
        ax.plot(time_q_real,z_qs, marker='o', markersize=4, label='Real Data')
        ax.scatter(225*dt,q_comsol[2,2] , s= 200, c='#d60000', marker='x',  label='Static Solution from COMSOL')
