# ------------------------------------------------------------------------------
# Benchmark against real beam (3cm x 3cm x 10cm)
# ------------------------------------------------------------------------------
...
class BeamEnv(EnvBase):
    def __init__(self, seed, folder, options):
        ...
        ### Search the specific points associated to the motion capture system. See the picture in Gitlab for more information to the        
        '''
        self.target_points=[
            [-0.094,0.041,  0.027],
            [-0.09, 0.041,  0.003],
        ...
        print_info(f"Left point: {verts[self.target_idx[1]]}")
    ...
    def _display_mesh(self, mesh_file, file_name):
        ...
        if self._mesh_type == 'tet':
            ...
            # Display the mesh vertices corresponding to motion markers with a red sphere
            for idx in self.target_idx:
                renderer.add_shape_mesh({
                    'name': 'sphere',
                    'center': ndarray(mesh.py_vertex(idx)),
...
if __name__ == '__main__':
    seed = 42
    np.random.seed(seed)
    folder = Path('real_beam_gravity_V2')

    ### Motion Markers data
    # Loading real data, units in meters
    import c3d
    qs_real = []
    reader = c3d.Reader(open('Measurement_data/beam_gravity_V2_b.c3d', 'rb'))
    ...
    # Manually select the relevant frames. For data a:
    #start_frame = 71
    #end_frame = 230
    # For data b:
    start_frame = 65
    ...
    #youngs_modulus_tet = 36184.41# parameter after optimization
    #youngs_modulus_hex = 356473.96# parameter after optimization
    ...
    hex_params = {
        ...
        'state_force_parameters': state_force,
        'mesh_type': 'hex',
        'refinement': 2.35#1.8# for 2052 DOFs#1.4 #for 1050 DOFs #2.35  #for 4608 DOFs #* 2.4 (=5.64) # for 32400 DOFs#1.5 #for 2050 DOFs
    }
    ...
    ### Simulation
    print_info(f"DoFs: {tet_deformable.dofs()} for Tet and {hex_deformable.dofs()} for Hex")
    ...
    for method, opt in zip(methods[:1], opts[:1]):
        ...
        print_info(f"Average time per frame: {1000*info_tet['forward_time']/frame_num:.2f}ms")
        ...
        '''
        q_comsol=np.array([[-48.765, 6.0553, 26.768],
            [-50.846, -0.053259, 2.9212],
            [-49.314, 29.944, 20.852],
            [0.56937, -0.0011645,    15.748],
            [1.1780, 15.000, 21.715],
            [-1.2364, 29.999, -2.1614]])
        '''

        #  Computation from Comsol for E=254830
        '''
        q_comsol=np.array([[-48.952, 6.0468, 27.269],
            [-50.707, -0.045390, 3.3860],
            [-49.408, 29.953, 21.336],
            [0.52615, -0.0010137,    17.009],
            [1.0423, 15.000, 22.985],
            [-1.0053, 29.999, -0.92597]])
        '''
        ...
        for i in range(frame_num+1):
            ...
            z_tet_i = qs_tet[i].reshape(-1,3).take(tet_env.target_idx_tip_left, axis=0)[:,2]-(qs_tet[0].reshape(-1,3).take(tet_env.target_idx_tip_left, axis=0)[:,2]-0.024)
            z_hex_i = qs_hex[i].reshape(-1,3).take(hex_env.target_idx_tip_left, axis=0)[:,2]-(qs_hex[0].reshape(-1,3).take(hex_env.target_idx_tip_left, axis=0)[:,2]-0.024)
            z_qs_i = qs_real[i,1,2]

            z_tet.append(z_tet_i)
            z_hex.append(z_hex_i)
            z_qs.append(z_qs_i)
        print_info(f"z_hex: {z_hex}")

        # Plot
        import matplotlib.pyplot as plt
        fig, ax = plt.subplots(figsize=(12,8))
        ax.plot(time, z_tet, marker='o', markersize=4, label='Tet Mesh ({} DoFs)'.format(tet_deformable.dofs()))
        ax.plot(time, z_hex, marker='o', markersize=4, label='Hex Mesh ({} DoFs)'.format(hex_deformable.dofs()))
        ...
        major_ticks_y = np.arange(0.0090, 0.025, 0.002)
        minor_ticks_y = np.arange(0.0090, 0.025, 0.0004)
        ax.set_yticks(major_ticks_y)
        ax.set_yticks(minor_ticks_y, minor=True)
        ...
        for i in range(frame_num+1):
            ...
            z_tet_i = qs_tet[i].reshape(-1,3).take(tet_env.target_idx_tip_right, axis=0)[:,2]-(qs_tet[0].reshape(-1,3).take(tet_env.target_idx_tip_right, axis=0)[:,2]-0.006)
            z_hex_i = qs_hex[i].reshape(-1,3).take(hex_env.target_idx_tip_right, axis=0)[:,2]-(qs_hex[0].reshape(-1,3).take(hex_env.target_idx_tip_right, axis=0)[:,2]-0.006)
            z_qs_i = qs_real[i,1,2]
        ...
        ax.plot(time, z_hex, marker='o', markersize=4, label='Hex Mesh ({} DoFs)'.format(hex_deformable.dofs()))
        ax.plot(time, z_qs, marker='o', markersize=4, label='Real Data')
        ax.scatter(155*dt,q_comsol[2,2] , s= 200, c='#d60000', marker='x',  label='Static Solution from COMSOL')
        ...
        major_ticks_y = np.arange(-0.01, 0.0076, 0.0025)
        minor_ticks_y = np.arange(-0.01, 0.0076, 0.0005)
        ax.set_yticks(major_ticks_y)
        ax.set_yticks(minor_ticks_y, minor=True)
        plt.xticks(fontsize=22)
        plt.yticks(fontsize=22)
        ...
        ax.set_title("Tetrahedral vs Hexahedral z Position of Right Tip Point vs Real Data (dt={}s)".format(dt), fontsize=28)
        ax.set_xlabel("Time [s]", fontsize=24)
        ax.set_ylabel("z Position [m]", fontsize=24)
        ax.title.set_position([.5, 1.03])
        ax.legend(loc="lower center", bbox_to_anchor=(0.5, -0.35), ncol= 2, prop={'size': 24})
