...
if __name__ == '__main__':
    seed = 42
    np.random.seed(seed)
    folder = Path('real_beam_ds_10_load_2_V2')
    ...
    import c3d
    qs_real = []
    reader = c3d.Reader(open('Measurement_data/beam_load101_V2_a.c3d', 'rb'))
    ...
    hex_params = {
        ...
        'state_force_parameters': state_force,
        'mesh_type': 'hex',
        'refinement': 2.35 * 1.4 #(for 9900 DOFs)
    }
    ...
    for method, opt in zip(methods[:1], opts[:1]):
        ...
        '''
        q_comsol=np.array([[-48.053, 29.692, 13.168],
            [-26.116, 6.2979, 18.508],
            [-53.831, -0.27985, -3.8018],
            [1.3330, 15.046, -3.8406],
            [-9.1564, 30.042, -24.262],
            [-1.6742, -0.21827, -8.1586]])
        '''

        #  Computation from Comsol for E=254830
        '''
        q_comsol=np.array([[-48.207, 29.739, 14.832],
            [-46.533, 6.2536, 20.309],
            [-53.157, -0.24079, -2.4190],
            [1.8980, 15.036, 1.2653],
            [-7.1545, 30.035, -20.022],
            [-0.73352, -0.19241, -3.4340]])
        '''

        #  Computation from Comsol for E=263834
        q_comsol=np.array([
        ...
        major_ticks_y = np.arange(-0.020, 0.027, 0.005)
        minor_ticks_y = np.arange(-0.020, 0.027, 0.001)
        ax.set_yticks(major_ticks_y)
        ax.set_yticks(minor_ticks_y, minor=True)
        ...
        ax.plot(time,z_hex, marker='o', markersize=4, label='Hex Mesh ({} DoFs)'.format(hex_deformable.dofs()))
        ax.plot(time,z_qs, marker='o', markersize=4, label='Real Data')
        ax.scatter(240*dt,q_comsol[2,2] , s= 200, c='#d60000', marker='x',  label='Static Solution from COMSOL')
        ...
        major_ticks_y = np.arange(-0.035, 0.0076, 0.0050)
        minor_ticks_y = np.arange(-0.035, 0.0076, 0.0010)
        ax.set_yticks(major_ticks_y)
        ax.set_yticks(minor_ticks_y, minor=True)
        plt.xticks(fontsize=22)
        plt.yticks(fontsize=22)
        ...
        major_ticks_y = np.arange(-0.035, 0.0076, 0.005)
        minor_ticks_y = np.arange(-0.035, 0.0076, 0.001)
        ax.set_yticks(major_ticks_y)
        ax.set_yticks(minor_ticks_y, minor=True)
        plt.xticks(fontsize=18)
        plt.yticks(fontsize=18)
