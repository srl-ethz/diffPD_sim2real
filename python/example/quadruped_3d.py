...
if __name__ == '__main__':
    ...
    #
    # # Set up antagonistic muscle pairs with random values
    # for key, indcs in leg_indices.items():
    #     if key[-1] == 'B':
    #         for idx in indcs:
    #             val = np.random.random()*act_max
    #             front = body_y_length*body_z_length*refinement^2 + 2*leg_z_length*refinement^2
    #             a0[idx] = val
    #             a0[idx+front] = act_max - val
    #
    #
    ...
    x_lb = ndarray([0, 2*np.pi / frame_num])#, 0])
    x_ub = ndarray([1, 2*np.pi / 5])#, np.pi/2])
    x_init = ndarray([np.random.random(), np.random.uniform(2*np.pi/frame_num, 2*np.pi/5)])
    bounds = scipy.optimize.Bounds(x_lb, x_ub)
    def loss_and_grad(x):
        A = x[0]
        w = x[1]
        #phi = x[2]
        jac = [np.ones((2, act_dofs)) for _ in range(frame_num)]
        jac_mult = [np.zeros((2, 2)) for _ in range(frame_num)]
            ...
            for key, indcs in leg_indices.items():
                if key[-1] == 'F':
                    ...
                    for idx in indcs:
                        a[i][idx] = act_max * (1 - A*np.sin(w*i)) / 2
                        jac[i][:, idx] = [-1, -1]
                jac_mult[i][0,0] = act_max * np.sin(w*i) / 2
                jac_mult[i][1,1] = act_max * A*i*np.cos(w*i) / 2
        env = QuadrupedEnv3d(seed, folder, { 'refinement': refinement,
            'youngs_modulus': youngs_modulus,
            'poissons_ratio': poissons_ratio,
            'leg_z_length': leg_z_length,
        ...
        grad = ndarray([jac_mult[i].dot(jac[i]).dot(np.transpose(act_grad[i])) for i in range(frame_num)])
        grad = np.sum(grad, axis=0)

        forward_time = info['forward_time']
        backward_time = info['backward_time']
        print('loss: {:8.3f}, |grad|: {:8.3f}, A: {:3.3f}, w: {:8.3f}, forward time: {:6.3f}s, backward time: {:6.3f}s'.format(loss, np.linalg.norm(grad), A, w, forward_time, backward_time))
        return loss, grad
    ...
    x_final = result.x
    print_info('Optimizing finished in {:6.3f} seconds'.format(t1 - t0))
        ...
        for key, indcs in leg_indices.items():
            if key[-1] == 'F':
                for idx in indcs:
                    ...
                    a_final[i][idx] =  act_max * (1 + A_final*np.sin(w_final*i)) / 2
            else:
                for idx in indcs:
                    a_init[i][idx] =  act_max * (1 - A_init*np.sin(w_init*i)) / 2
                    a_final[i][idx] =  act_max * (1 - A_final*np.sin(w_final*i)) / 2
