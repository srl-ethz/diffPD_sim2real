        ...
        if data_file.exists():
            ...
            for method in ['newton_pcg', 'newton_cholesky', 'pd_eigen']:
                ...
                for d in data[method]:
                    ...
                    print('loss: {:8.3f}, |grad|: {:8.3f}, |x|: {:8.3e}, forward time: {:6.3f}s, backward time: {:6.3f}s'.format(
                        d['loss'], np.linalg.norm(d['grad']), np.linalg.norm(d['x']), d['forward_time'], d['backward_time']))
