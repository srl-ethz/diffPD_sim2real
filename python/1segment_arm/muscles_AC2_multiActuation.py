...
if __name__ == '__main__':
    ...
    pressure = 450
    startendframes = [40, 98]

    seed = 42
    folder = Path(f'Muscles_Design_AC2_{pressure}_multiActuation')
    ...
    def variable_to_act(x):
        ...
        for t in range(frame_num):
            ...
            frame_act_1 = np.concatenate([
                np.ones(len(fiber)) * (1+2*x[i_c]) for fiber in hex_env.fibers_1
            ])
            frame_act_2 = np.concatenate([
                np.ones(len(fiber)) * (1-x[i_c]) for fiber in hex_env.fibers_2
            ])
    ...
    ### Optimization
    x_lb = np.ones(control_frame_num) * 0
    x_ub = np.ones(control_frame_num) * 2

    #init_x = 0.31
    x_init = np.linspace(0.1, 0.6, control_frame_num)
