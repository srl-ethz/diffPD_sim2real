...
if __name__ == '__main__':
    for pressure, startendframes, init_a in zip([100, 150, 200, 250, 300, 350, 400, 450], [[59, 140], [67, 148], [79, 160], [77, 158], [155, 240], [78, 140], [50, 146], [40, 98]], [1.05, 1.12, 1.19, 1.28, 1.40, 1.58, 1.80, 1.95]):
        seed = 42
        ...
        # for 100mbar:
        #x_init = np.ones(frame_num) *1.05

        # for 150mbar:
        #x_init = np.ones(frame_num) *1.12

        #for 200mbar:
        #x_init = np.ones(frame_num) * 1.19

        #for 250mbar:
        #x_init = np.ones(frame_num) * 1.28

        #for 300mbar
        #x_init = np.ones(frame_num) * 1.40

        #for 350mbar
        #x_init = np.ones(frame_num) * 1.58

        # for 400mbar:
        x_init = np.ones(frame_num) *1.8#1.68#* 1.8 is working for 400mbar and E=130000 and E=263824 (same value!), but we have falting of the muscles, 1.68 for E=130000
