...
class ArmEnv(EnvBase):
    def __init__(self, seed, folder, options):
        ...
        for idx in range(element_num):
            ...
            if vy < 0.014 and abs(vz-(max_corner[2]))<0.002:
...
if __name__ == '__main__':
    ...
    #for 200mbar:
    #x_init = np.ones(frame_num) * 0.84
    #for 300mbar
    #x_init = np.ones(frame_num) * 0.68
    # for 400mbar:
    x_init = np.ones(frame_num) * 0.44
    ...
    print_info("Creating a combined video (it can take a few minutes)...")
    ...
    for i in range(0, frame_num+1):
        ...
        options = {
            ...
            #'camera_lookat': (0, 0, .2)     # Position that camera looks at
            #'camera_pos': (1.1, 0.15, 0.9),  # Position of camera (0.7, -1.2,0.8)
            #'camera_lookat': (0, 0, .2) 
    ...
    def display_with_info_custom(folder_name, frame_num, fps, dt):
        ...
        # Create an image containing the wanted information
        img = Image.new('RGB', (350, 80), color = (73, 109, 137))
        d=ImageDraw.Draw(img)
        font=ImageFont.truetype("DejaVuSans-Bold.ttf", 18)
        ...
        #d.text((10,70), "Pink: Actuation Elements", font=font)
