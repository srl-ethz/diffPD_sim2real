...
class ArmEnv(EnvBase):
    def __init__(self, seed, folder, options):
        ...
        ## Check the boundary coordinates of the struture
        min_corner = np.min(verts, axis=0)
        max_corner = np.max(verts, axis=0)
        print_info("min corner start: {}".format(min_corner))
        print_info("max corner start: {}".format(max_corner))

        ## Center of Mass of all elements
        self.com_elements=[]
        ...
        ## First find the number of fibers and attribute one element at the top of each fiber ("actuatio_top_el")
        self.count_fibers=0
        ...
        for idx in range(element_num):
            ...
            if vy> 0.03 and abs(vz-(max_corner[2]))<0.002:
        ...
        verts = verts @ R.T
        elements = elements @ R.T
        

        min_corner = np.min(verts, axis=0)
        max_corner = np.max(verts, axis=0)
        #print_info("min corner after rotation: {}".format(min_corner))
        #print_info("max corner after rotation: {}".format(max_corner))
        ...
        print_info("min corner after rotation and translation: {}".format(min_corner_after))
        print_info("max corner after rotation and translation: {}".format(max_corner_after))
    ...
    def display_with_info_custom(self, folder_name, frame_num, fps, dt):
        ...
        img = Image.new('RGB', (320, 60), color = (73, 109, 137))
        ...
        d.text((10,20), "Pink: Muscle Extension")
        d.text((10,30), f"Real time: {fps*dt}")
...
if __name__ == '__main__':
    seed = 42
    folder = Path('Muscles_Design_CRC')
    ...
    #for 200mbar:
    #x_init = np.ones(frame_num) * 1.16
    #for 300mbar
    #x_init = np.ones(frame_num) * 1.3
    # for 400mbar:
    x_init = np.ones(frame_num) * 1.68
    ...
    def variable_to_act(x):
        ...
        for t in range(len(x)):
            frame_act = np.concatenate([
                np.ones(len(chamber)) * x[t] for chamber in hex_env.fibers
            ])
    ...
    np.savetxt(f"{folder}/distance_{deformable.dofs()}.csv", distance, delimiter =",",fmt ='% s')
    ...
    for i in range(0, frame_num+1):
        ...
        options = {
            ...
            'sample': 4,
            'max_depth': 2,
            #'camera_pos': (0.7, -1.2, 1.3),  # Position of camera (0.7, -1.2,0.8)
            #'camera_lookat': (0, 0, .2)     # Position that camera looks at
            #'camera_pos': (1.1, 0.3, 1.1),  # Back View
            #'camera_lookat': (0, 0, .2)     # Back View
            'camera_pos': (0.7, -0.7, 0.9),  # Normal View
            'camera_lookat': (0, 0, .2)     # Normal View
            #'camera_pos': (0.5, -1., 1.6),  # Position of camera (0.7, -1.2,0.8)
            #'camera_lookat': (0, 0, .2)     # Position that camera looks at
        }
        renderer = PbrtRenderer(options)
        ...
        for idx in hex_env.target_idx[:2]:
            renderer.add_shape_mesh({
                'name': 'sphere',
    ...
    def display_with_info_custom(folder_name, frame_num, fps, dt):
        ...
        #d.text((10,50), "Yellow: Hex Mesh Solution (Ref Pt)", font=font)
